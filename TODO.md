...
更新时间：`2026-08-15T17:04:13+08:00`（UTC+8）  
...
- [ ] 为实际 Cocos 游戏项目生成 `PROJECT-<name>.md` 独立约束。
- [ ] 配置 OCR 引擎（如 PaddleOCR/Tesseract）并完成真实截图识别联调。
- [ ] 配置网关 WSS 地址与鉴权，验证长连接重连与上下文记忆。
- [ ] 联调 cc-switch/ccs 路由模式连接真实编辑器。
- [ ] 为 CLI 核心模块补充单元测试。
- [ ] 在 Cocos Creator 3.8 中安装扩展并验证面板 CLI 窗口。
- [ ] 建立 CI/预提交检查（tsc、lint、测试、文档链接）。
