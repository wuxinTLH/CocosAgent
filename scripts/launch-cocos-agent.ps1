...
$existingConfig = @{ cliIndex = [IO.Path]::GetFullPath($cliIndex); version = 'v0.0.0.8-a'; overlay = $true; creatorPath = '' }
if (Test-Path -LiteralPath $configFile -PathType Leaf) {
    ...
    } catch { $existingConfig = @{ cliIndex = [IO.Path]::GetFullPath($cliIndex); version = 'v0.0.0.8-a'; overlay = $true; creatorPath = '' } }
}
$existingConfig.cliIndex = [IO.Path]::GetFullPath($cliIndex)
$existingConfig.version = 'v0.0.0.8-a'
